...
const CMS_CONFIG = {
    postsUrl: 'CMS/posts.json' // Path to posts.json relative to the usage, or absolute
};
        ...
        <article class="glass-card p-8 rounded-2xl hover:border-brand-accent/30 transition-colors">
            <header class="mb-6">
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-white hover:text-brand-accent transition-colors">
                    <a href="post.html?slug=${post.slug}" class="block">${post.title}</a>
                </h2>
            </header>
            ...
            <div class="mt-6">
                <a href="post.html?slug=${post.slug}" class="inline-flex items-center text-brand-accent hover:underline font-medium group">
                    Read Full Article
    ...
    if (!post) {
        container.innerHTML = `
            <div class="text-center py-20">
                ...
                <a href="blog.html" class="text-brand-accent hover:underline">Return to Blog</a>
            </div>
        `;
    ...
    container.innerHTML = `
        <article>
            ...
            <div class="flex justify-center">
                <a href="blog.html" class="inline-flex items-center px-6 py-3 border border-slate-700 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-all">
